...
class SupabaseModel(BaseModel):
    ...
    def create(cls, **values: Any) -> Self:
        ...
        client = get_client()
        ins = client.table(cls.__table__).insert(payload).execute()
        if not ins.data:
    ...
    @classmethod
    def bulk_create(cls, rows: list[dict[str, Any]]) -> list[Self]:
        if not rows:
            return []
    ...
    def upsert(
        ...
        **values: Any,
        ...
        resp = get_client().table(cls.__table__).upsert(payload, **kw).execute()
        if not resp.data:
