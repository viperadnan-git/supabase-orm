...
def test_default_query_class_is_querybuilder():
    from supabase_orm._async import QueryBuilder
...
async def test_upsert_flat_uses_upsert_response_directly(fake_client):
    pid = uuid4()
    fake_client.queue(
        FakeResponse(
            data=[
...
async def test_upsert_ignore_duplicates_passes_through(fake_client):
    fake_client.queue(
        FakeResponse(
            data=[{"id": str(uuid4()), "name": "a", "species": "cat", "adopted": False}]
        )
    )
    await Pet.upsert(name="a", species="cat", adopted=False, ignore_duplicates=True)
    builder = fake_client.builders[0]
    upsert_call = next(c for c in builder.calls if c[0] == "upsert")
    assert upsert_call[2]["ignore_duplicates"] is True


async def test_upsert_returns_no_rows_raises(fake_client):
    fake_client.queue(FakeResponse(data=[]))
    with pytest.raises(ValueError, match="returned no rows"):
...
async def test_bulk_upsert_empty_list_no_call(fake_client):
    assert await Pet.bulk_upsert([]) == []
    assert fake_client.builders == []
...
async def test_get_or_create_returns_existing_when_found(fake_client):
    pid = uuid4()
    fake_client.queue(
        FakeResponse(
            data=[
    ...
    p, created = await Pet.get_or_create(species="cat", defaults={"name": "X"})
...
async def test_get_or_create_creates_when_missing(fake_client):
    pid = uuid4()
    ...
    p, created = await Pet.get_or_create(
        species="cat",
...
async def test_update_or_create_updates_when_found(fake_client):
    pid = uuid4()
    ...
    p, created = await Pet.update_or_create(
        species="cat",
...
async def test_update_or_create_creates_when_missing(fake_client):
    pid = uuid4()
    fake_client.queue(
        FakeResponse(data=[]),
        FakeResponse(
...
async def test_upsert_accepts_typed_column(fake_client):
    pid = uuid4()
    fake_client.queue(
        FakeResponse(
    ...
    await Pet.upsert(
        id=pid, name="x", species="cat", adopted=False, on_conflict=Pet.f.id
    )
    upsert_call = next(c for c in fake_client.builders[0].calls if c[0] == "upsert")
    assert upsert_call[2]["on_conflict"] == "id"


async def test_upsert_accepts_list_of_columns_for_composite(fake_client):
    pid = uuid4()
    fake_client.queue(
        FakeResponse(
            data=[{"id": str(pid), "name": "x", "species": "cat", "adopted": False}]
...
async def test_upsert_typo_in_column_raises_at_call_site():
...
async def test_bulk_upsert_accepts_column_list_for_composite(fake_client):
    fake_client.queue(
        FakeResponse(
            data=[
                {"id": str(uuid4()), "name": "A", "species": "cat", "adopted": False},
