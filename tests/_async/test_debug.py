...
from supabase_orm._async import ExplainResult, SupabaseModel, set_client
...
async def test_logger_disabled_by_default(fake_client, caplog):
    fake_client.queue(FakeResponse(data=[]))
    with caplog.at_level(logging.WARNING, logger=_LOGGER_NAME):
        await Pet.query.eq("name", "x").all()
    assert [r for r in caplog.records if r.name == _LOGGER_NAME] == []


async def test_logger_emits_method_path_and_ms_when_debug(pg_client, caplog):
...
async def test_log_path_swallows_introspection_failure(fake_client, caplog):
    ...
    with caplog.at_level(logging.DEBUG, logger=_LOGGER_NAME):
        rows = await Pet.query.eq("name", "x").all()
    assert len(rows) == 1
    records = [r for r in caplog.records if r.name == _LOGGER_NAME]
