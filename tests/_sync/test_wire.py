# DO NOT EDIT — generated from tests/_async/test_wire.py by scripts/gen_sync.py.
...
def test_query_construction_passes_full_select(fake_client):
    fake_client.queue(FakeResponse(data=[]))
    Row.query.eq("id", uuid4()).all()
...
def test_filter_op_routes_to_correct_postgrest_method(
    fake_client, method, col, value, expected_call, expected_args
):
    fake_client.queue(FakeResponse(data=[]))
    getattr(Row.query, method)(col, value).all()
    call = _only(fake_client.builders[0], expected_call)
    assert call[1] == expected_args


def test_match_dict_routes_to_postgrest_match(fake_client):
    fake_client.queue(FakeResponse(data=[]))
    Row.query.match({"name": "a", "is_active": True}).all()
    call = _only(fake_client.builders[0], "match")
    assert call[1] == ({"name": "a", "is_active": True},)


def test_match_validates_columns(fake_client):
    with pytest.raises(AttributeError):
        Row.query.match({"nope": 1})


def test_match_serializes_values(fake_client):
    ...
    Row.query.match({"id": u, "created_at": dt}).all()
    call = _only(fake_client.builders[0], "match")
    assert call[1] == ({"id": str(u), "created_at": dt.isoformat()},)


def test_match_counts_as_filter_for_bulk_guard(fake_client):
    fake_client.queue(FakeResponse(data=[]))
    # Should NOT raise — match() is a narrowing op.
    Row.query.match({"is_active": True}).delete()


def test_in_routes_to_in_(fake_client):
    fake_client.queue(FakeResponse(data=[]))
    Row.query.in_("n", [1, 2, 3]).all()
...
def test_text_search_variants_via_querybuilder(fake_client, method, options):
    fake_client.queue(FakeResponse(data=[]))
    getattr(Row.query, method)("name", "cat").all()
...
def test_eq_serializes_uuid(fake_client):
    fake_client.queue(FakeResponse(data=[]))
    u = uuid4()
    Row.query.eq("id", u).all()
    assert _only(fake_client.builders[0], "eq")[1] == ("id", str(u))


def test_eq_serializes_datetime(fake_client):
    fake_client.queue(FakeResponse(data=[]))
    dt = datetime(2024, 1, 2, 3, 4, 5, tzinfo=timezone.utc)
    Row.query.eq("created_at", dt).all()
    assert _only(fake_client.builders[0], "eq")[1] == ("created_at", dt.isoformat())


def test_eq_serializes_date(fake_client):
    fake_client.queue(FakeResponse(data=[]))
    d = date(2024, 1, 2)
    Row.query.eq("due", d).all()
    assert _only(fake_client.builders[0], "eq")[1] == ("due", "2024-01-02")


def test_eq_serializes_decimal(fake_client):
    fake_client.queue(FakeResponse(data=[]))
    Row.query.eq("amount", Decimal("1.25")).all()
    assert _only(fake_client.builders[0], "eq")[1] == ("amount", "1.25")


def test_eq_serializes_enum(fake_client):
    fake_client.queue(FakeResponse(data=[]))
    Row.query.eq("status", Status.ACTIVE).all()
    assert _only(fake_client.builders[0], "eq")[1] == ("status", "active")


def test_in_serializes_each_element_mixed_types(fake_client):
    fake_client.queue(FakeResponse(data=[]))
    u1, u2 = uuid4(), uuid4()
    Row.query.in_("id", [u1, u2]).all()
...
def test_or_predicate_string_per_operator(fake_client, method, col, value, expected):
    fake_client.queue(FakeResponse(data=[]))
    Row.query.or_(lambda q, m=method, c=col, v=value: getattr(q, m)(c, v)).all()
    or_call = _only(fake_client.builders[0], "or_")
    assert or_call[1] == (expected,)


def test_or_predicate_with_in_uses_paren_tuple(fake_client):
    fake_client.queue(FakeResponse(data=[]))
    Row.query.or_(lambda q: q.in_("n", [1, 2, 3])).all()
    or_call = _only(fake_client.builders[0], "or_")
...
def test_or_predicate_serializes_uuid(fake_client):
    fake_client.queue(FakeResponse(data=[]))
    u = uuid4()
    Row.query.or_(lambda q: q.eq("id", u)).all()
    or_call = _only(fake_client.builders[0], "or_")
    assert or_call[1] == (f"id.eq.{u}",)


def test_or_predicate_quotes_value_with_comma(fake_client):
    fake_client.queue(FakeResponse(data=[]))
    Row.query.or_(lambda q: q.eq("name", "a,b")).all()
    or_call = _only(fake_client.builders[0], "or_")
    # Same wrap rule applies: branch with a comma → wrapped.
    assert or_call[1] == ('and(name.eq."a,b")',)


def test_not_predicate_wraps_in_not_and(fake_client):
    fake_client.queue(FakeResponse(data=[]))
    Row.query.not_(lambda q: q.gt("n", 5).lt("n", 10)).all()
    or_call = _only(fake_client.builders[0], "or_")
...
def test_order_by_emits_in_chain_order(fake_client):
    fake_client.queue(FakeResponse(data=[]))
    Row.query.eq("is_active", True).order_by("-created_at", "name").all()
...
def test_limit_offset_range_pass_through(fake_client):
    fake_client.queue(FakeResponse(data=[]))
    Row.query.eq("is_active", True).limit(10).offset(5).range(0, 9).all()
    b = fake_client.builders[0]
...
def test_first_appends_limit_1(fake_client):
    fake_client.queue(FakeResponse(data=[]))
    Row.query.eq("is_active", True).first()
    assert _only(fake_client.builders[0], "limit")[1] == (1,)


def test_one_and_maybe_one_use_limit_2(fake_client):
    fake_client.queue(
        FakeResponse(
            data=[
                _row(uuid4()),
    ...
    Row.query.eq("is_active", True).one()
    assert _only(fake_client.builders[0], "limit")[1] == (2,)


def test_chained_limit_caps_terminal_and_sends_one_param(fake_client):
    fake_client.queue(FakeResponse(data=[_row(uuid4())]))
    Row.query.eq("is_active", True).limit(1).maybe_one()
    assert _only(fake_client.builders[0], "limit")[1] == (1,)


def test_chained_limit_wider_than_terminal_keeps_terminal_cap(fake_client):
    fake_client.queue(FakeResponse(data=[]))
    Row.query.eq("is_active", True).limit(50).first()
    assert _only(fake_client.builders[0], "limit")[1] == (1,)


def test_chained_range_capped_by_terminal(fake_client):
    fake_client.queue(FakeResponse(data=[]))
    Row.query.eq("is_active", True).range(10, 19).first()
    assert _only(fake_client.builders[0], "range")[1] == (10, 10)


def test_exists_respects_chained_limit(fake_client):
    fake_client.queue(FakeResponse(data=[]))
    Row.query.eq("is_active", True).limit(1).exists()
    assert _only(fake_client.builders[0], "limit")[1] == (1,)


def test_all_with_count_passes_count_exact(fake_client):
    fake_client.queue(FakeResponse(data=[], count=0))
    Row.query.eq("is_active", True).all_with_count()
...
def test_create_payload_fully_serialized(fake_client):
    pid = uuid4()
    dt = datetime(2024, 1, 2, 3, 4, 5, tzinfo=timezone.utc)
...
def test_bulk_create_payload_fully_serialized(fake_client):
    a, b = uuid4(), uuid4()
...
def test_save_serializes_dirty_fields_and_pk_in_eq(fake_client):
    pid = uuid4()
    r = Row(
        id=pid,
        name="n",
...
def test_instance_update_serializes_payload(fake_client):
    pid = uuid4()
    r = Row(
        id=pid,
        name="n",
    ...
    fake_client.queue(FakeResponse(data=[_row(pid)]))
    r.update(status=Status.ARCHIVED, amount=Decimal("2.50"))
...
def test_query_update_serializes_payload(fake_client):
    fake_client.queue(FakeResponse(data=[]))
    dt = datetime(2026, 1, 1, tzinfo=timezone.utc)
    Row.query.eq("is_active", True).update(
...
def test_delete_eq_pk_serializes_value(fake_client):
    pid = uuid4()
    r = Row(
        id=pid,
        name="n",
...
def test_compound_query_wire_shape(fake_client):
    fake_client.queue(FakeResponse(data=[]))
