# DO NOT EDIT — generated from tests/_async/test_predicates.py by scripts/gen_sync.py.
...
from supabase_orm._sync import Column, Predicate, SupabaseModel
...
def test_or_accepts_predicate_args(fake_client):
...
def test_or_predicate_with_and_branch(fake_client):
    fake_client.queue(FakeResponse(data=[]))
    Row.query.or_(
        Row.f.name == "a",
...
def test_or_predicate_with_nested_or(fake_client):
    fake_client.queue(FakeResponse(data=[]))
    Row.query.or_(
...
def test_not_accepts_predicate(fake_client):
    fake_client.queue(FakeResponse(data=[]))
    Row.query.not_(Row.f.is_active == True).all()  # noqa: E712
    or_call = next(c for c in fake_client.builders[0].calls if c[0] == "or_")
    assert or_call[1] == ("not.and(is_active.eq.true)",)


def test_not_of_or_group(fake_client):
    fake_client.queue(FakeResponse(data=[]))
    Row.query.not_((Row.f.name == "a") | (Row.f.name == "b")).all()
    or_call = next(c for c in fake_client.builders[0].calls if c[0] == "or_")
    assert or_call[1] == ("not.or(name.eq.a,name.eq.b)",)


def test_or_requires_at_least_one_branch(fake_client):
    from supabase_orm._sync import SupabaseORMUsageError

    with pytest.raises(SupabaseORMUsageError, match="at least one branch"):
        Row.query.or_()


def test_or_rejects_mixed_predicate_and_lambda(fake_client):
    from supabase_orm._sync import SupabaseORMUsageError
...
def test_or_legacy_lambda_path_still_works(fake_client):
    fake_client.queue(FakeResponse(data=[]))
    Row.query.or_(
...
def test_not_legacy_lambda_path_still_works(fake_client):
    fake_client.queue(FakeResponse(data=[]))
    Row.query.not_(lambda q: q.eq("name", "a")).all()
    or_call = next(c for c in fake_client.builders[0].calls if c[0] == "or_")
...
def test_predicate_or_composes_with_chain_filters(fake_client):
...
def test_column_asc_returns_order():
    from supabase_orm._sync import Order
...
def test_order_by_string_form_unchanged(fake_client):
    """The ``"-col"`` shorthand still works exactly as before."""
    fake_client.queue(FakeResponse(data=[]))
    Row.query.order_by("-age", "name").all()
...
def test_order_by_typed_column_defaults_to_asc(fake_client):
    fake_client.queue(FakeResponse(data=[]))
    Row.query.order_by(Row.f.age).all()
    orders = [c for c in fake_client.builders[0].calls if c[0] == "order"]
    assert orders == [("order", ("age",), {"desc": False})]


def test_order_by_typed_desc(fake_client):
    fake_client.queue(FakeResponse(data=[]))
    Row.query.order_by(Row.f.age.desc()).all()
    orders = [c for c in fake_client.builders[0].calls if c[0] == "order"]
    assert orders == [("order", ("age",), {"desc": True})]


def test_order_by_with_nulls_position(fake_client):
    fake_client.queue(FakeResponse(data=[]))
    Row.query.order_by(
...
def test_order_by_mixes_string_and_typed_forms(fake_client):
    fake_client.queue(FakeResponse(data=[]))
    Row.query.order_by("-age", Row.f.name).all()
    orders = [c for c in fake_client.builders[0].calls if c[0] == "order"]
    assert orders == [
        ("order", ("age",), {"desc": True}),
        ("order", ("name",), {"desc": False}),
